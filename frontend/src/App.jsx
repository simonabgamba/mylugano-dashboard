...
KPI: ${label}. Data: ${ctx}`,
    system_prompt: (s, notes, sheetContext) => `You are a senior data analyst for MyLugano, the digital wallet and cashback platform of the City of Lugano, Switzerland, launched in 2021.
...
Answer concisely and professionally in English. CRITICAL: use ONLY the data provided above — never invent or extrapolate numbers not explicitly present. If the data needed is not available, say so clearly and ask up to 3 clarifying questions.`,
  },
  it: {
...
KPI: ${label}. Dati: ${ctx}`,
    system_prompt: (s, notes, sheetContext) => `Sei un analista senior di MyLugano, la piattaforma di wallet digitale e cashback della Città di Lugano, lanciata nel 2021.
...
${notes && Object.keys(notes).length > 0 ? "NOTE DEL TEAM: " + Object.entries(notes).map(([k,v]) => k+": "+v).join(", ") + "." : ""}

Rispondi in italiano, in modo conciso e professionale. Non usare markdown con asterischi. IMPORTANTE: usa ESCLUSIVAMENTE i dati forniti sopra — non inventare o estrapolare numeri non presenti. Se i dati necessari non sono disponibili, dillo chiaramente e fai fino a 3 domande di chiarimento.`,
  }
};
...
function ExportBtn({ cardRef, title, compact }) {
  ...
  async function doExport() {
    ...
    try {
      ...
      const walkText = (el, depth) => {
        ...
        } else { children.forEach(c => walkText(c, depth + 1)); }
      };
      Array.from(node.children).forEach(child => {
        if (!child.querySelector("svg") && child.tagName !== "BUTTON") walkText(child, 0);
      });
  ...
  return (
    <button onClick={doExport} disabled={exporting} style={{
      marginTop: compact ? 0 : 12, fontSize: compact ? 10 : 11,
      color: exporting ? RED : MUTED, background: "none", border: `1px solid ${BORDER}`,
      borderRadius: compact ? 6 : 8, padding: compact ? "3px 8px" : "4px 12px",
...
function YearFilter({ allYears, selected, onChange, t }) {
  return (
    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16, alignItems: "center" }}>
      ...
      {allYears.map(y => {
        ...
        return (
          <button key={y} onClick={() => {
            ...
            color: active ? color : "#aaa", cursor: "pointer", fontWeight: active ? 700 : 400
          }}>{y}</button>
        );
      })}
      ...
      <button onClick={() => onChange([])} style={{ fontSize: 11, padding: "3px 10px", borderRadius: 12, border: `1px solid ${BORDER}`, background: "#fff", color: MUTED, cursor: "pointer" }}>{t.none}</button>
    </div>
  );
}
...
function isLocalPeak(pivoted, mese, year) {
  ...
  const prev = pivoted[idx-1][key], curr = pivoted[idx][key], next = pivoted[idx+1][key];
...
function TrendEndLabel({ points, color, pct }) {
  ...
  const sign = pct >= 0 ? "+" : "";
  return <text x={last.x + 6} y={last.y} textAnchor="start" fontSize={9} fontWeight={700} fill={color}>{sign}{pct}%</text>;
}

function CustomBarLabel(props) {
  const { x, y, width, value } = props;
  if (!value || value <= 0) return null;
  return <text x={x + width / 2} y={y - 4} textAnchor="middle" fontSize={8} fontWeight={600} fill={MUTED}>{value >= 1000 ? (value / 1000).toFixed(1) + "k" : value}</text>;
}
...
function KpiCard({ label, value, delta, prev, pos, context, kpiPromptFn, t }) {
  ...
  useEffect(() => {
    ...
    callClaude(kpiPromptFn(label, context)).then(text => {
      try { const clean = text.replace(/```json|```/g, "").trim(); setAnalysis(JSON.parse(clean)); }
...
function NoteTooltip({ active, payload, label, years, notes }) {
  ...
  return (
    <div style={{ background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 10, padding: "10px 14px", fontSize: 12, maxWidth: 220 }}>
      ...
      {payload.map((p, i) => p.value && <div key={i} style={{ color: p.color, marginBottom: 2 }}>{p.name}: {p.value.toLocaleString()}</div>)}
        ...
        <LineChart data={pivoted} margin={{ bottom: noteMesi.length > 0 ? 16 : 0 }}>
          ...
          {years.map(y => {
            ...
            if (validVals.length < 2) return (
              <Line key={y} type="monotone" dataKey={"y"+y} stroke={YEAR_COLORS[y]||RED} strokeWidth={2}
                dot={<CustomDot pivoted={pivoted} dataKey={"y"+y} />} name={String(y)} connectNulls={false}
            ...
            const firstV = validVals[0], lastV = validVals[validVals.length-1];
            const trendPct = firstV ? Math.round((lastV-firstV)/firstV*100) : null;
            return (
              <React.Fragment key={y}>
                <Line type="monotone" dataKey={"y"+y} stroke={YEAR_COLORS[y]||RED} strokeWidth={2}
                  dot={<CustomDot pivoted={pivoted} dataKey={"y"+y} />} name={String(y)} connectNulls={false}
                  strokeDasharray={y===new Date().getFullYear()?"4 2":undefined}>
                ...
                <Line type="linear" dataKey={trendKey} stroke={YEAR_COLORS[y]||RED} strokeWidth={1} dot={false}
...
function CustomDotAllTime(props) {
  const { cx, cy, index, value, serie, showEvery3 } = props;
...
function ChartAllTime({ data, dataKey, title, sub, showEvery3, notes, yPadding }) {
  ...
  const yMax = yPadding ? Math.ceil((maxVal + yPadding) / 10000) * 10000 : undefined;
  const noteKeys = notes ? serie.reduce((acc, d, i) => { const k = d.mese+"-"+d.anno; if (notes[k]) acc.push(i); return acc; }, []) : [];
    ...
    <Card exportTitle={title}>
      ...
      <ResponsiveContainer width="100%" height={240}>
        <LineChart data={serieWithTrend}>
          <CartesianGrid strokeDasharray="3 3" stroke={BORDER} />
          <XAxis dataKey="label" tick={<CustomXTickAllTime noteLabels={noteKeys.map(i => serie[i]?.label)} />}
            ticks={[...serie.filter((_,i) => i%6===0).map(d=>d.label), ...noteKeys.map(i=>serie[i]?.label)].filter((v,i,arr)=>arr.indexOf(v)===i)}
            height={noteKeys.length > 0 ? 40 : 20} />
          <YAxis tickFormatter={v => v ? (v/1000).toFixed(0)+"k" : ""} tick={{ fontSize: 10 }} domain={yMax ? [0, yMax] : ['auto','auto']} />
...
function buildQuarterly(data) {
  ...
  return Object.entries(map)
    .sort(([a],[b]) => { const [qa,ya]=a.split(" "), [qb,yb]=b.split(" "); return ya!==yb ? parseInt(ya)-parseInt(yb) : qa.localeCompare(qb); })
...
function ChartNuoviUtenti({ data, years, title, lang, kpiPromptFn, t }) {
  ...
  useEffect(() => {
    ...
    callClaude(kpiPromptFn(lang==="it" ? "Nuovi utenti per trimestre" : "New users per quarter", ctx)).then(text => {
      try { const clean = text.replace(/```json|```/g,"").trim(); setAnalysis(JSON.parse(clean)); }
      catch { setAnalysis(null); }
      setLoading(false);
    }).catch(() => setLoading(false));
  }, [lang, JSON.stringify(serie)]);

  const serieWithTrend = serie.map((d,i) => {
    const n=serie.length, xMean=(n-1)/2;
  ...
  const trendPct = serie.length>=2 ? Math.round((serieWithTrend[serie.length-1].trend-serieWithTrend[0].trend)/serieWithTrend[0].trend*100) : null;
  ...
  return (
    <Card style={{ gridColumn: "1 / -1", marginBottom: 14 }} exportTitle={title}>
      ...
      <ResponsiveContainer width="100%" height={220}>
        <ComposedChart data={serieWithTrend} barSize={22}>
          <CartesianGrid strokeDasharray="3 3" stroke={BORDER} />
...
function ChatBot({ summary, notes, sheetContext, t }) {
  ...
  async function send() {
    ...
    setLoading(true);
    try {
      const system = t.system_prompt(summary, notes, sheetContext);
      const reply = await callClaude(userMsg, system);
...
export default function App() {
    ...
    async function fetchAll() {
      try {
        const [s, u, r, tx, d, n, cfg, ctx] = await Promise.all([
          ...
          fetch(`${API}/api/context`).then(r => r.json()).catch(() => ({})),
        ]);
        setSummary(s);
  ...
  const gridStyle = { display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 14 };
    ...
    <div style={{ background: "#f7f7f9", minHeight: "100vh", fontFamily: "system-ui,sans-serif", padding: isMobile ? "1rem" : "2rem 1.5rem 4rem" }}>
      ...
      <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch", marginBottom: "1.5rem" }}>
        <div style={{ display: "flex", gap: 8, minWidth: "max-content", paddingBottom: 4 }}>
          {[["kpi",t.tab_kpi],["trends",t.tab_trends],["history",t.tab_history],["chat",t.tab_chat]].map(([id,label]) => (
            <button key={id} onClick={() => setTab(id)} style={{
              fontSize: 13, fontWeight: tab===id ? 600 : 400, padding: "8px 20px", borderRadius: 20, whiteSpace: "nowrap",
              border: tab===id ? `1.5px solid ${RED}` : `1px solid ${BORDER}`,
              background: tab===id ? RED_L : "#fff", color: tab===id ? RED : MUTED, cursor: "pointer"
            }}>{label}</button>
          ))}
        </div>
      </div>
        ...
        <>
          ...
          <div style={gridStyle}>
            {t.kpis.map(k => {
              ...
              return (
                <KpiCard key={k.key} label={k.label} value={s?.valore?.toLocaleString()}
                  delta={`${s?.delta_pct > 0 ? "+" : ""}${s?.delta_pct}% vs prev`}
                  prev={`${s?.prev?.toLocaleString()} prev month`}
                  pos={s?.delta_pct >= 0} context={ctx}
                  kpiPromptFn={(label, ctx) => t.kpi_prompt(label, ctx, notes)} t={t} />
              );
            })}
          </div>
        </>
      ...
      {tab === "trends" && (
        <>
          ...
          <div style={gridStyle}>
            <ChartLine data={users} dataKey="utenti" years={selYears} title={lang==="it" ? "Utenti totali" : "Total Users"} sub={lang==="it" ? "Trend mensile cumulativo" : "Monthly cumulative trend"} yPadding={20000} notes={notes} />
            <ChartLine data={users} dataKey="wallet_attivi" years={selYears.filter(y=>y>=2025)} title={lang==="it" ? "Wallet attivi" : "Active Wallets"} sub={lang==="it" ? "Disponibile dal 2025" : "Available from 2025"} notes={notes} />
            <ChartBar data={revenue} dataKey="incassi_chf" years={selYears} title={lang==="it" ? "Ricavi mensili CHF" : "Monthly Revenue CHF"} sub={lang==="it" ? "Circuito totale" : "Total circuit"} notes={notes} />
            <ChartBar data={revenue} dataKey="cashback_chf" years={selYears} title={lang==="it" ? "Cashback emesso CHF" : "Cashback Issued CHF"} sub={lang==="it" ? "Circuito totale" : "Total circuit"} notes={notes} />
            <ChartBar data={transactions} dataKey="transazioni" years={selYears} title={lang==="it" ? "Transazioni mensili" : "Monthly Transactions"} sub={lang==="it" ? "Circuito totale" : "Total circuit"} notes={notes} />
            <ChartLine data={downloads} dataKey="download_totali" years={selYears.filter(y=>y>=2025)} title={lang==="it" ? "Download cumulativi" : "Cumulative Downloads"} sub="iOS + Android" notes={notes} />
          </div>
      ...
      {tab === "history" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 14 }}>
          <ChartAllTime data={users} dataKey="utenti" title={lang==="it" ? "Utenti — storico completo" : "Users — full history"} sub="2021 →" showEvery3={true} notes={notes} yPadding={20000} />
          <ChartAllTime data={transactions} dataKey="transazioni" title={lang==="it" ? "Transazioni — storico completo" : "Transactions — full history"} sub="2024 →" notes={notes} />
          <ChartAllTime data={revenue} dataKey="incassi_chf" title={lang==="it" ? "Ricavi CHF — storico completo" : "Revenue CHF — full history"} sub="2024 →" notes={notes} />
          <ChartAllTime data={revenue} dataKey="cashback_chf" title={lang==="it" ? "Cashback CHF — storico completo" : "Cashback CHF — full history"} sub="2024 →" notes={notes} />
        </div>
      )}
